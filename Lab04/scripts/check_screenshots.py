...
def main() -> int:
    parser = argparse.ArgumentParser(description="Kiểm tra ảnh thủ công của Lab04.")
    ...
    paths = [p for p in folder.iterdir() if p.is_file() and p.name != ".gitkeep"]
    ...
    for path in sorted(paths):
        ...
        if path.suffix.lower() != ".png" or path.stat().st_size == 0:
            problems.append(f"EMPTY OR WRONG FORMAT: {path.name}")
            continue
        try:
