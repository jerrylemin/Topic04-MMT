...
- Tài liệu vận hành: `README.md`, `HUONG_DAN_CHUP_ANH.md` đủ 28 kịch bản và 13 trường hướng dẫn cho mỗi ảnh.
...
- `scripts/generate_report.py` sinh DOCX/PDF tiếng Việt theo `compact_reference_guide` + `editorial_cover`, 17 chương + phụ lục, TOC, số trang, caption và placeholder ảnh chi tiết.
...
- WSL: `python scripts/check_screenshots.py` - đúng 0/28; thiếu toàn bộ ảnh thủ công, không có ảnh thừa/hỏng/trùng.
- WSL: `python scripts/generate_report.py` - tạo thành công DOCX/PDF với 28 placeholder.
...
- Chụp thủ công 28 PNG theo `HUONG_DAN_CHUP_ANH.md`, chạy lại checker và report generator.
