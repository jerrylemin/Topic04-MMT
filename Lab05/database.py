...
def execute_read_only(sql: str, parameters: tuple = ()):
    # ponytail: the demo exposes only fixed SELECT builders; sqlite3.execute also rejects stacked statements.
    if not sql.lstrip().upper().startswith("SELECT"):
        raise ValueError("Lab05 read-only query required")
    before = get_db().total_changes
    rows = get_db().execute(sql, parameters).fetchall()
    if get_db().total_changes != before:
        get_db().rollback()
        raise RuntimeError("Read-only demonstration modified the database")
    return rows
