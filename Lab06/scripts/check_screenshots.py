...
def png_dimensions(path: Path) -> tuple[int, int]:
    ...
    while offset+12<=len(data):
        ...
        if dims is None:
            ...
            if not all(dims): raise ValueError("kích thước PNG bằng 0")
        if kind==b"IEND": iend=True; break
        offset=end
    if dims is None or not iend: raise ValueError("PNG thiếu IEND")
    return dims
def validate_screenshots(directory: Path) -> list[str]:
    errors=[]; expected=set(EXPECTED_FILES); actual={p.name for p in directory.iterdir() if p.is_file()} if directory.is_dir() else set()
    errors += [f"THIẾU: {n}" for n in sorted(expected-actual)]; errors += [f"THỪA: {n}" for n in sorted(actual-expected)]
    hashes={}
    ...
    errors += [f"TRÙNG HASH: {', '.join(v)}" for v in hashes.values() if len(v)>1]
    return errors
def main(argv=None)->int:
    if hasattr(sys.stdout,"reconfigure"): sys.stdout.reconfigure(encoding="utf-8")
    parser=argparse.ArgumentParser(description=f"Kiểm tra {len(EXPECTED_FILES)} ảnh thủ công Lab06"); parser.add_argument("directory",nargs="?",type=Path,default=Path(__file__).resolve().parents[1]/"evidence"/"screenshots"); parser.add_argument("--list-required",action="store_true"); args=parser.parse_args(argv)
    ...
    if errors: print(f"Bộ ảnh chưa đạt ({len(errors)} lỗi):"); [print(f"- {e}") for e in errors]; return 1
